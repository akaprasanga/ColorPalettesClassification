...
class WidgetGallery(QDialog):
    ...
    def crete_parameter_groupbox(self):
        ...
        self.file_open_btn = QPushButton('Open Image')
        selected_color_text = QLabel("Selected Color")
        result_count_txtlbl = QLabel("Define Number of Output Palettes")
        self.result_count_spinbox = QSpinBox()
        self.result_count_spinbox.setValue(200)
        self.result_count_spinbox.setMaximum(300)
        self.result_count_spinbox.setMinimum(2)
        self.result_count_spinbox.setSingleStep(50)
        self.selected_color_lbl = QLabel()
        ...
        vbox_layout.addWidget(self.selected_color_lbl, 1, 0)
        vbox_layout.addWidget(filter_group_box, 2, 0)
        vbox_layout.addWidget(result_count_txtlbl, 3, 0)
        vbox_layout.addWidget(self.result_count_spinbox, 4, 0)
    ...
    def set_status_to_thread(self):
        self.worker_process.result_count = int(self.result_count_spinbox.value())
        self.worker_process.filter_string = self.get_filter_string()
        self.worker_process.selected_color = tuple(self.selected_color_value)
...
class WorkerThread(QThread):
    ...
    def run(self):
        ...
        sorting_obj = SortingDatabase(filename="3000ColorValues.xlsx", c=tuple(self.selected_color), filter_string=self.filter_string, number=self.result_count)
        output_result = sorting_obj.handle_requests()
        return_list.append(output_result)
